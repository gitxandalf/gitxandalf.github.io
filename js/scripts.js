...
$(document).ready(function () {
	...
	if (iOS == false) {
		if (!device.tablet() && !device.mobile()) {
			...
			$('#skill').bind('inview', function (event, visible) {
				if (visible === true) {
					var container = $("#skill .page-title h1"); container.shuffleLetters();
				}
			});
	...
	if (!device.mobile()) {
		...
		var ut_word_rotator = function () {
			var ut_rotator_words5 = [
				'Building the future, and enjoying every second',
				'Staying true to my roots, living in nature'
			],
				counter = 0;
			setInterval(function () {
				$(".gallery-text-rotate5").fadeOut(function () { $(this).html(ut_rotator_words5[counter = (counter + 1) % ut_rotator_words5.length]).fadeIn(); });
			}, 4000);
		}
		ut_word_rotator();
		...
		/* Gallery block-8 Word Rotator
			================================================== */
		var ut_word_rotator = function () {
			var ut_rotator_words8 = [
				'Always working hard and playing hard',
				'Keeping what\'s important close to heart'
			],
				counter = 0;
			setInterval(function () {
				$(".gallery-text-rotate8").fadeOut(function () { $(this).html(ut_rotator_words8[counter = (counter + 1) % ut_rotator_words8.length]).fadeIn(); });
			}, 6000);
		}
		ut_word_rotator();
		...
		================================================== */
		var ut_word_rotator = function () {
			var ut_rotator_words16 = [
				'Staying humble and mindful while pushing boundaries',
				'Practicing the basics while simultaneously trying new things'
			],
				counter = 0;
			setInterval(function () {
				$(".gallery-text-rotate16").fadeOut(function () { $(this).html(ut_rotator_words16[counter = (counter + 1) % ut_rotator_words16.length]).fadeIn(); });
			}, 5000);
		}
		ut_word_rotator();
